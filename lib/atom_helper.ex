defmodule AtomHelper do
  ...
  """
  @spec atom_to_str(atom | map | list) :: (binary | map | list)
  ...
  @spec str_to_atom(binary | map | list) :: (atom | map | list)
  ...
  @spec str_to_atom(binary | %{binary=>binary} | list, boolean) :: %{atom=>atom}
  def str_to_atom(map, conv_val) when is_map(map), do: for {key, val} <- map, into: %{}, do:
    {String.to_atom(key), (if conv_val, do: String.to_atom(val), else: val)}
